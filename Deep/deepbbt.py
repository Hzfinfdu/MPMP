...
save_path = 'deepbbt_results/{}_results/range_{}_budget_{}_{}_seed_{}_{}_{}_{}'.format(
    task_name,
    bound,
    budget_router,
    budget_prompt,
...
class LMForwardAPI:
    def __init__(self):
        ...
        self.best_prompt = pretrained_state['z'].clone()
    ...
    def set_target_param(self, param, is_router=False, interest_index=None):
        if is_router:
            self.model.model.model.encoder.encoder.router.data = param.view(1, -1, 8)
        else:
            self.model.model.model.encoder.encoder.z.data[interest_index] = param.tile(8, 1)
...
def get_best_z_composition(idx):
    ...
    return torch.mm(router.unsqueeze(0), model_forward_api.model.model.model.encoder.encoder.z[idx]).squeeze().detach().cpu().numpy().tolist()

cma_opts = {
    'seed': seed,
    'popsize': popsize,
    'maxiter': budget_prompt // (popsize * 24),
    'verbose': -1,
}
if bound > 0:
...
for _ in range(budget_prompt // (popsize * 24)):
    for i, es in enumerate(es_list):
        solutions = es.ask()
        fitnesses = [model_forward_api.eval(x, interest_index=i) for x in solutions]
        es.tell(solutions, fitnesses)
        model_forward_api.set_target_param(torch.tensor(es.result.xbest, device=device, dtype=torch.float32), interest_index=i)
